...
class AtlasAutoencoder:
    ...
    def set_learning_rate(self, lr: float):
        """Set the learning rate of all per-chart optimizers (polish phase)."""
    ...
    def differential_reconstruction_loss(
        self,
        autoencoder: LocalAutoencoder,
        x: tf.Tensor,
    ) -> tf.Tensor:
        """
        Differential reconstruction loss: mean ||d(E∘D)_z - I_d||_F².

        Training-time analogue of the differential reconstruction condition
        (Definition of approximate atlas, item (4)): reconstruction loss
        controls ε but nothing in the loss controls η — charts routinely
        converge in ε while η > 1 on several charts. This term penalises
        the latent round-trip Jacobian directly. Opt-in via lambda_diff.
        """
        ...
        J = tape.batch_jacobian(z_round, z)  # (batch, d, d)
        I_d = tf.eye(self.latent_dim, batch_shape=[tf.shape(z)[0]])
        return tf.reduce_mean(tf.reduce_sum((J - I_d) ** 2, axis=[1, 2]))
    ...
    def train_step(
        ...
        for i, (ae, opt) in enumerate(zip(self.autoencoders, self.optimizers)):
            ...
            with tf.GradientTape() as tape:
                ...
                if lambda_diff > 0:
                    L_diff = self.differential_reconstruction_loss(ae, x)
                    loss = loss + lambda_diff * L_diff
                    losses[f"diff_{i}"] = L_diff
    ...
    def fit(
        ...
        Args:
            ...
            lambda_diff: Weight for differential reconstruction loss
                (penalises ||d(E∘D) - I||²; targets the η hypothesis directly)
