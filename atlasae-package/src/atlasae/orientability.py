...
def _get_pair_entries(
    results: Dict,
    chart_i: int, comp_i: int,
    chart_j: int, comp_j: int,
) -> List[Tuple[int, np.ndarray]]:
    """
    All (sign, overlap_point_indices) entries for a pair of chart
    components, over every connected component of their overlap.
    Handles both key orders (ω_ji = ω_ij for real line/determinant signs).
    """
    entries = []
    for key, sign in results['signs'].items():
        if key[:4] == (chart_i, comp_i, chart_j, comp_j) or \
           key[:4] == (chart_j, comp_j, chart_i, comp_i):
            entries.append((sign, results['overlaps'][key]))
    return entries
...
def verify_cocycle_condition(
    ...
    """
    Verify the cocycle condition ω_ik = ω_ij · ω_jk on common triple overlaps.

    The identity det g_ik = det g_ij · det g_jk holds POINTWISE on
    U_i ∩ U_j ∩ U_k. It is therefore only checked on triples of overlap
    components that share at least `min_common` sample points. Checking it
    on component triples WITHOUT a common region is incorrect: around a
    non-contractible loop of a non-orientable manifold the product of
    pairwise signs is legitimately -1 — that is the w₁ signal (the Möbius
    mechanism), not a violation. The unrestricted check rejects precisely
    the well-trained atlases that represent the twist faithfully.

    Violations on genuine common triple overlaps indicate numerical issues
    ...
    Args:
        ...
        min_common: Minimum shared sample points for a triple of overlap
            components to be subject to the check

    Returns:
        (cocycle_verified, violations): Boolean and list of violating triples
    """
    print("\n" + "="*60)
    print("STEP 3: Verifying Cocycle Condition (on common triple overlaps)")
    print("="*60)

    violations = []
    n_checked = 0
    n_skipped_no_common = 0
    ...
    for idx_i in range(n_comps):
        ...
        for idx_j in range(idx_i + 1, n_comps):
            ...
            for idx_k in range(idx_j + 1, n_comps):
                ...
                entries_ij = _get_pair_entries(results, chart_i, ci, chart_j, cj)
                entries_jk = _get_pair_entries(results, chart_j, cj, chart_k, ck)
                entries_ik = _get_pair_entries(results, chart_i, ci, chart_k, ck)

                if not entries_ij or not entries_jk or not entries_ik:
                    continue

                # Cocycle condition per combination of overlap components,
                # restricted to their common intersection
                for sign_ij, idx_ij in entries_ij:
                    for sign_jk, idx_jk in entries_jk:
                        common_ij_jk = np.intersect1d(idx_ij, idx_jk)
                        if len(common_ij_jk) < min_common:
                            n_skipped_no_common += 1
                            continue
                        for sign_ik, idx_ik in entries_ik:
                            common = np.intersect1d(common_ij_jk, idx_ik)
                            ...
                            if sign_ik == expected:
                                if verbose:
                                    print(f"  ✓ ({chart_i}.{ci}, {chart_j}.{cj}, "
                                          f"{chart_k}.{ck}) on {len(common)} common pts: "
                                ...
                                if verbose:
                                    print(f"  ✗ ({chart_i}.{ci}, {chart_j}.{cj}, "
                                          f"{chart_k}.{ck}) on {len(common)} common pts: "
                                          f"ω_ij·ω_jk = {sign_ij}·{sign_jk} = {expected} "
                                          f"≠ {sign_ik} = ω_ik")

    if n_skipped_no_common and verbose:
        print(f"  ({n_skipped_no_common} component triples skipped: "
              f"no common triple overlap — not subject to the cocycle identity)")
